      ...
      methods: {
        ...
        getTravelAgency(){
          details(this.userId).then(response=>{
            //console.log({{ $t(devServer.proxy["/api"].target) }})
            //console.log( devServer.proxy["/api"].target)
            var path = require('../../../vue.config')
            this.options.img = path.devServer.proxy["/api"].target + response.data.guideUserBasic.coverphoto;
          })
        },
        // 编辑头像
        editCropper() {
