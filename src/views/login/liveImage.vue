      ...
      methods: {
        ...
        getTravelAgency(){
          details(this.userId).then(response=>{
            console.log(response.data.guideUserBasic.lifephoto)
            var path = require('../../../vue.config')
            this.options.img = path.devServer.proxy["/api"].target + response.data.guideUserBasic.lifephoto;
          })
        },
        // 编辑头像
        editCropper() {
            ...
            uploadAvatar(formData).then(response => {
              if (response.code === 0) {
                ...
                this.$router.push({
                  path: '/infoPerfection',
                  query: {
                    liveImgUrl: response.data.avatar
                  }
                })
              } else {
                alert("上传失败");
